...
Session Helper
--------------
Real accounts now exist (`services/account_service.py`, ported from
...
pages use to drive the auth UI (see `app/pages/Login.py`).
...
# services/account_service.py::Account for what these fields mean and
...
def set_anonymous_onboarding_profile(profile: dict) -> None:
    ...
    instead (see app/pages/Onboarding.py, which decides which of the
    two to call).
    """
