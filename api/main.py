...
# Guarantee a stable JWT_SECRET_KEY exists before any request can reach
# a register/login call, even on a fresh checkout that never had .env
# set up - see utils/tokens.py::ensure_jwt_secret for exactly what this
# does and does not touch. Runs before the `logging`/router imports
# below so it can never race a request.
from utils.tokens import ensure_jwt_secret  # noqa: E402
...
def create_app() -> FastAPI:
    ...
    # Signed session cookie, used only to carry the OAuth `state` value
    # between /auth/{provider}/login and its callback. Nothing else is
    # kept in it, and it is not an alternative to the JWT: the access
    # token is still what authenticates every API call.
    app.add_middleware(
        SessionMiddleware,
        secret_key=os.environ.get("SESSION_SECRET_KEY", ""),
        session_cookie="dwai_oauth_session",
        same_site="lax",  # must survive the provider's cross-site redirect back
        https_only=False,  # local development is plain http
        max_age=600,  # a sign-in round-trip is seconds; 10 min is generous
    )
    ...
    app.include_router(insights_cards.router, prefix=settings.api_prefix)
    app.include_router(badges.router, prefix=settings.api_prefix)
    app.include_router(league_chat.router, prefix=settings.api_prefix)
    app.include_router(journal.router, prefix=settings.api_prefix)
    app.include_router(personal.router, prefix=settings.api_prefix)
