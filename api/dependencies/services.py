...
from services.ml.persona_service import PersonaService
...
def get_prediction_service(
    ...
    # services/ml/prediction_service.py's own docstring on this) so a new
...
def get_cohort_service_cls() -> type[CohortService]:
    ...
    # a module-level functools.lru_cache in services/ml/cohort_service.py),
...
def get_plan_side_storage_backend() -> StorageBackend | None:
    """Same idea as get_history_storage_backend, for the three stores
    that hang off the weekly plan: the frozen plan snapshot, the
    out-of-band day decisions, and the violation ledger.

    Deliberately a SEPARATE dependency rather than reusing the history
    one. HistoryService selects its records by user_id alone, so
    pointing these at the same backend would make a stored plan
    snapshot show up as a check-in - a day with no date and no score
    appearing in somebody's history. They are different kinds of
    record and they get different stores.

    Returns None in production, which lands each service on its own
    DEFAULT_STORAGE_PATH exactly as before. Tests override it so a run
    never writes to the real storage/*.json files."""
    return None
