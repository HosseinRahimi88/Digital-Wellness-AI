...
def analytics_summary(
    ...
    # Exception days are read separately: they are drawn on the chart as
    # markers but must never enter the statistics computed below.
    exception_dates = {
        e.get("date")
        for e in history_service.get_all(include_excluded=True)
    ...
    score_history = [
        ScoreHistoryPoint(
            ...
            is_exception=row["date"].strftime("%Y-%m-%d") in exception_dates,
        )
