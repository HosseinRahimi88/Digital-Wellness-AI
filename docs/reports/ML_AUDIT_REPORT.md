...
| Target reconstruction | `models/data_loader.py` reconstructs `health_score_0_100` (missing from the raw CSVs) as a weighted mean of the six composite subscores plus a seventh screen-load subscore — a legitimate, documented target-engineering step, not leakage, since all seven are excluded from the feature set. The screen-load term was added after this audit: the six subscores describe how a person slept and felt and none of them measures how long the screen was on, so the reconstructed score correlated **+0.034** with recreational screen minutes — no relationship, and the wrong sign, in an application about digital load. `utils/screen_load.py` measures the seventh subscore against published thresholds (a two-hour recreational guideline and a logistic dose-response curve whose steep band is the 2–4 h range the literature names), and the correlation is now **−0.754** while the correlation with the six wellbeing subscores stays at **+0.577**. Sources and the weight measurement are in that module and in `models/data_loader.py`. |
...
caught: every existing test fixture (`tests/wellness/test_recommendation_service.py`'s
