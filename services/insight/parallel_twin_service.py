...
predict forward in time - see legacy/streamlit_app/pages/Dashboard.py's own docstring
...
(services/insight/dashboard_service.py::DashboardService.split_shap_by_direction,
...
# legacy/streamlit_app/components/what_if_simulator.py exposes to a human, restricted
...
class ParallelTwinService:
    ...
    def build_twin_profile(
        ...
        for shap_feature in areas_to_improve:
            ...
            # Move a fraction of the way toward the healthy target, and
            # stop there. The same defect fixed in
            # services/insight/future_path_service.py lived here in miniature: a
            # flat percentage shift with no ceiling can carry a field
            # past the point where it is still healthy, and the twin
            # then scores worse than the user - which reads as "your
            # better habits made you worse". The shift is small here
            # (15%, three fields), so it was not visible on the shipped
            # profiles, but the shape of the bug was identical.
            target = HEALTHY_TARGETS.get(field)
            if target is None:
                continue
            already_there = current <= target if direction == "decrease" else current >= target
            if already_there:
                continue

            new_value = current + TWIN_SHIFT_FRACTION * (target - current)
            new_value = (
                max(new_value, target) if direction == "decrease" else min(new_value, target)
            )
