...
from config.recommendation_i18n import localized_text, personalise, priority_i18n, safety_note_i18n
...
class RecommendationService:
    ...
    def generate(
        ...
        category_track_record
            ...
            services.wellness.recommendation_effectiveness_service
        ...
        for feature in harmful_features:
            ...
            # C-2. Attach this user's own number and the target the
            # rule's fixed formula produces from it, plus the four
            # languages. Nothing here is random and nothing consults a
            # model - same input, same output, every time. When
            # `user_data` is not supplied (older call sites) the
            # recommendation is exactly what it was before.
            numbers = personalise(feature.feature, user_data or {})
            recommendation.observed_value = numbers["observed"]
            recommendation.target_value = numbers["target"]
            recommendation.text_i18n = localized_text(feature.feature, numbers)
            recommendation.priority_i18n = priority_i18n(recommendation.priority)
            recommendation.safety_note_i18n = safety_note_i18n(recommendation.safety_note)
