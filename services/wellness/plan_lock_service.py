...
class LockedPlan:
    ...
    # "model" or "constant" - how the stored band's width was arrived
    # at, frozen with the band itself. None for a snapshot written
    # before the band model existed, and reported as None rather than
...
class PlanLockService:
    ...
    def weeks(self) -> list[str]:
        """Every week this user has a stored plan for, oldest first.

        The snapshots were already being written one per ISO week - a
        23-day demo leaves four of them - but nothing could list them, so
        the weekly page could only ever show the current week and the
        earlier ones were unreachable. Sorting is plain string order,
        which is chronological for ISO "YYYY-Www" keys.
        """
        keys = {
            record.get("week_key")
            for record in self._backend.read_all()
            if record.get("user_id") == self.user_id and record.get("week_key")
        }
        return sorted(keys)
    ...
    def _to_locked(record: dict[str, Any]) -> Optional[LockedPlan]:
        ...
        return LockedPlan(
            ...
            band_source=record.get("band_source"),
...
# --------------------------------------------------------------- score band
# The half-width used when no trained band model is available. It was
# the only behaviour until services/ml/band_model_service existed, and
# it is still the floor the whole feature degrades to: no artifact, an
# artifact that will not load, a user with no usable day - all of them
# land back here rather than on an error.
#
# It is also, measurably, a bad constant. Against this dataset's real
# per-respondent sequences it covers 98.2% of days, so a day falls
# outside it about once every 78 days and the exception question is
# asked roughly once every eleven weeks. Keeping it as the fallback is
# a deliberate choice to fail wide - a band that is too generous asks
# too few questions, where one that is too tight questions everything.
BAND_HALF_WIDTH = 6.0
...
def week_band(
    ...
    (see services/wellness/day_decision_service.py). Omitted, every day counts
    once, which is what every caller did before exceptions existed.

    `days` are the stored history entries behind those scores, same
    order, and they are only ever used to make the WIDTH personal - the
    centre is the weighted mean either way, and every other weekly-plan
    rule is untouched by them.

    `half_width` forces a width instead of asking the model. Tests that
    are about the arithmetic rather than about the model pass 6.0 here
    and stay independent of whether an artifact happens to be present.

    The width itself comes from services/ml/band_model_service, which
    predicts how far THIS person's next day tends to fall from their own
    running mean. Its coverage is conformal-calibrated on held-out
    respondents; when it has no opinion, BAND_HALF_WIDTH applies.
    """
    low, high, _, _ = week_band_detail(scores, weights, days, half_width)
    return (low, high)


def week_band_detail(
    scores: list[float],
    weights: Optional[list[float]] = None,
    days: Optional[list[dict]] = None,
    half_width: Optional[float] = None,
    ...
    """week_band(), plus the width it used and where that width came from.

    `prior_scores` is everything this user logged BEFORE this week,
    oldest first. It is what lets the width be genuinely personal: the
    days since Monday cannot tell a volatile person from a steady one,
    and the model was under-covering exactly those people until it
    could see further back. Omitted, the band is what it was.

    `source` is "model" when services/ml/band_model_service produced the
    half-width and "constant" when it fell back to BAND_HALF_WIDTH. It
    is reported rather than left to be inferred from the number, for the
    same reason the cohort panel names its source: a reader is entitled
    to know whether the range in front of them was fitted to them or is
    the same one everybody gets.
    """
    pairs = [
    ...
    if half_width is not None:
        # An explicit width is the caller's own business - claiming it
        # came from the model would be a lie, and claiming it is the
        # constant would be one too if it is not 6.0.
        source = "explicit"
    else:
        # Imported here, not at module scope: this module is storage and
        # is imported by the demo seeder and by tests that have no
        # business paying for joblib and a pickle, and the model itself
        # is loaded lazily behind this call anyway.
        from services.ml import band_model_service

        half_width = band_model_service.half_width(scores, days, prior_scores)
        if half_width is not None:
            source = "model"
    ...
    return (
        round(max(0.0, avg - half_width), 1),
        round(min(100.0, avg + half_width), 1),
