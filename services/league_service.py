...
-----------------------
Peer-to-peer version of services/family_service.py's consent model,
...
DEFAULT_LEAGUE_PATH = Path(__file__).resolve().parent.parent / "storage" / "league.json"
...
class LeagueService:
    ...
    def __init__(self, backend: Optional[StorageBackend] = None) -> None:
        self._backend = backend or JSONFileStorageBackend(DEFAULT_LEAGUE_PATH)
    ...
    def get_or_create_profile(self, user_id: str) -> LeagueProfile:
        with self._backend.transaction() as records:
            ...
            profile = LeagueProfile(
                user_id=user_id, invite_code=secrets.token_hex(4).upper(), created_at_utc=_now_iso(),
            )
    ...
    def accept_rules(self, user_id: str) -> LeagueProfile:
        with self._backend.transaction() as records:
            for r in records:
                if self._is_profile(r) and r.get("user_id") == user_id:
                    ...
                    return self._profile_from_record(r)
            profile = LeagueProfile(
                user_id=user_id, invite_code=secrets.token_hex(4).upper(),
