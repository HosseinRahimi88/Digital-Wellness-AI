...
@dataclass
class RowPreview:
    """What one row scored, whether or not it was kept.

    In a dry run this is the entire point of the upload; in a real
    import it is confirmation that the days now in the history are the
    days the file described.
    """
    row_number: int
    date: str
...
class ImportResult:
    ...
    # Every row that validated and scored, in date order. Populated in
    # both modes - see `dry_run`.
    previews: list[RowPreview] = field(default_factory=list)
    # True when this import scored the file and deliberately stored
    # nothing. Echoed back so the caller cannot mistake a preview for a
    # save.
    dry_run: bool = False
...
class CSVImportService:
    ...
    def import_csv_text(
        ...
        * the single-row questionnaire CSV the result page exports, which
        ...
        `dry_run` scores the file and stores nothing. It exists because
        the duplicate-day rule was making the common case fail: someone
        who just wants to SEE what a file scores had no way to say so,
        so they uploaded, got "that day is already recorded", and were
        told to turn on an edit tick they did not want - the tick that
        overwrites the day they were trying not to touch. With a dry run
        there is nothing to overwrite, so the duplicate check is skipped
        entirely and no row can be refused for a reason that only
        matters when saving.
        """
        result = ImportResult(dry_run=bool(dry_run))
        rows = list(csv.DictReader(io.StringIO(csv_text)))
        existing_dates: set[str] = set()
        if not allow_update and not dry_run:
            try:
                existing_dates = {
                    entry.get("date")
        ...
        for i, raw_row in enumerate(rows, start=1):
            ...
            result.previews.append(RowPreview(
                row_number=i,
                date=date_str,
            ...
            if dry_run:
                # Scored, shown, and deliberately not kept. The row is
                # NOT counted as imported: `imported_count` is what the
                # caller uses to say "your history changed", and in a dry
                # run it did not.
                continue
