...
# entries for their own charts (see services/insight/analytics_service.py's
...
# referenced elsewhere (services/insight/opportunity_cost_service.py's
...
class HistoryService:
    ...
    def delete_users(self, user_ids: Iterable[str]) -> int:
        """Remove every entry belonging to any of `user_ids`, in ONE
        locked transaction. Returns how many rows went.

        `delete_all()` is per-user and rewrites the whole file each
        time, which is right for "delete my account" and wrong for
        Demo Mode: rebuilding a ten-friend demo deletes the demo user
        plus ten bots, i.e. eleven full rewrites of a file that holds
        every user's history.

        That cost is invisible on a fresh install and grows with total
        rows across ALL users. Measured on this machine: with 14,762
        rows (43 MB) a 23-day ten-friend demo took 31.4s; against empty
        storage the same demo took 5.3s. The frontend gives up at 45s,
        so the demo eventually stops working the more the app is used -
        which is exactly how it was reported.

        Same scoping rule as delete_all(): a row is only removed if its
        own user id is in the set, so this can never reach another
        account's data.
        """
        ...
        if not wanted:
            return 0
        with self.storage.transaction() as all_entries:
            remaining = [e for e in all_entries if self._entry_user_id(e) not in wanted]
            removed = len(all_entries) - len(remaining)
            if removed:
                self.storage.commit(remaining)
        logger.info(
            "Deleted %d history entries for %d user(s) in one transaction",
            removed, len(wanted),
        )
        return removed

    def record_many_users(
        self,
        rows_by_user: dict[str, list[tuple[dict[str, Any], Any, Optional[str], date]]],
    ) -> int:
        """`record_many()` for SEVERAL users at once, in one transaction.

        The demo's ten friends each called record_many() separately -
        ten more full rewrites on top of the deletes above. Returns the
        number of entries written.

        Upsert semantics are unchanged and still per (user, date): a
        user's existing row for a date being written is replaced, and
        every row belonging to anyone not in `rows_by_user` is left
        exactly as it was.
        """
        built: dict[str, dict[str, dict[str, Any]]] = {}
        for user_id, rows in (rows_by_user or {}).items():
            if not rows:
                continue
            service = HistoryService(user_id=user_id, storage=self.storage)
            by_date: dict[str, dict[str, Any]] = {}
            for user_data, prediction_result, persona, on_date in rows:
                entry = service._build_entry(user_data, prediction_result, persona, on_date)
                by_date[entry["date"]] = entry
            built[user_id] = by_date
        if not built:
            return 0
        ...
        with self.storage.transaction() as all_entries:
            remaining = [
                e for e in all_entries
                if not (
                    self._entry_user_id(e) in built
                    and e.get("date") in built[self._entry_user_id(e)]
                )
            ]
            written = 0
            for by_date in built.values():
                remaining.extend(by_date.values())
                written += len(by_date)
            remaining.sort(key=lambda e: (e.get("date", ""), self._entry_user_id(e)))
            self.storage.commit(remaining)
        ...
        logger.info(
            "Recorded %d history entries across %d users in one transaction",
            written, len(built),
        )
        return written
