...
class ImprovementPlanService:
    ...
    def generate(
        ...
        focus_rules = self._select_focus_rules(user_data, history, schedule_type)
        ...
        for i in range(6):
            ...
            level_index = min(occurrence + carried, len(levels) - 1)
            tier_label = _TIER_LABELS[min(level_index, len(_TIER_LABELS) - 1)] if len(levels) > 1 else ""

            composed = self._compose_tasks(rule, user_data, i, occurrence, carried)
            days.append(
                DailyPlan(
                    day_number=i + 1,
                    day_label=f"Day {i + 1}",
    ...
    def _compose_exercises(
        ...
        tier = TIERS[min(occurrence + max(0, carried), len(TIERS) - 1)]
        ...
        for offset in range(3):
            index = day_index + offset * slot_step + max(0, carried)
    ...
    def _compose_tasks(
        ...
        return [
            e["text"].get("en", "")
            for e in self._compose_exercises(rule, user_data, day_index, occurrence, carried)
            if e["text"].get("en")
        ]
    ...
    def signal_tracks(
        ...
        for rule in self._HABIT_RULES:
            ...
            if severity > 0:
                if schedule_type in self._IRREGULAR_SCHEDULES:
                    severity *= 1.0 + self._IRREGULAR_BOOST.get(rule["theme"], 0.0)
        ...
        strengthen.sort(key=lambda e: e.get("severity", 0.0), reverse=True)
    ...
    def _select_focus_rules(
        ...
        if needing_work:
            ...
            # rule. This re-ranks, it never invents a focus area that
            # the user's own numbers did not already flag.
            if schedule_type in self._IRREGULAR_SCHEDULES:
                needing_work = [
                    (sev * (1.0 + self._IRREGULAR_BOOST.get(rule["theme"], 0.0)), rule)
