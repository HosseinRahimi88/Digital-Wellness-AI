...
Run: python3 -m unittest tests.test_future_path_service -v
"""
...
from services.future_path_service import FuturePathService, PATH_DEFINITIONS, FuturePathComparison
...
class TestEffortOrdering(unittest.TestCase):
    ...
    def test_no_improvement_path_pushes_a_field_past_its_healthy_target(self):
        ...
        from services.future_path_service import HEALTHY_TARGETS
        from services.parallel_twin_service import TWIN_ADJUSTABLE_FIELDS
        for name, profile in self._profiles().items():
            paths, _ = self._scores(profile)
