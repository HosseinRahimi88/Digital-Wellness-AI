...
2. IMPORTANT SANDBOX NOTE: the execution environment these tests were
   ...
   package (an optional, heavier dependency of `services/ml/shap_service.py`)
   cannot be installed. To still exercise the *real, unmodified*
   `services/ml/shap_service.py` end-to-end (rather than skip/mock it), this
...
# THE one file that legitimately computes the project root itself.
#
# Every other test imports it from core/paths.py, and must - recomputing
# it from a file's own depth is what silently breaks the moment the tree
# is reorganised. This module is the exception because it is the thing
# that puts the project root ON sys.path in the first place: importing
# `core` to find out where `core` is would be circular.
#
# tests/_test_support.py -> tests/ -> the project root.
...
try:
    ...
    class _TreeExplainerUnavailable:
        ...
        `services/ml/shap_service.py`'s own real fallback logic already
...
# used by legacy/streamlit_app/pages/Prediction.py's "Quick Demo" tab) - kept here as
...
# ==========================================================
# Outbound mail, under test
# ==========================================================
#
# Registration now sends a verification code, so without this every
# suite that creates an account prints a full JWT and a mail body to the
# console through the log transport - hundreds of times over a full run,
# burying anything that actually matters.
#
# The memory transport is installed as the process-wide default here.
# A test that needs to READ a code still installs its own (see
# tests/test_auth_hardening._memory_mail) - this only stops the noise
# and guarantees no test can accidentally reach a real SMTP server
# because a developer happened to have SMTP_HOST set.
try:  # pragma: no cover - import-time wiring
    from services.identity.mail_service import (  # noqa: E402
        MailService as _MailService,
        MemoryTransport as _MemoryTransport,
        set_mail_service as _set_mail_service,
    )

    _set_mail_service(_MailService(_MemoryTransport()))
except Exception:  # noqa: BLE001 - a missing mail service must not break collection
    pass
