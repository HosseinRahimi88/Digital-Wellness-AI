...
Server behaviour lives in tests/test_plan_days_and_violations.py. This
...
Run: python3 -m unittest tests.test_plan_sequence_ui -v
"""
...
class TestPlanSequenceUI(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.weekly_js = (JS / "weekly.js").read_text(encoding="utf-8")
        cls.hall_js = (JS / "hall-page.js").read_text(encoding="utf-8")
    ...
    def test_a_refused_tick_rolls_the_box_back(self):
        idx = self.weekly_js.index("plan_day_locked_toast")
        block = self.weekly_js[max(0, idx - 700):idx + 200]
    ...
    def test_finishing_a_day_re_reads_the_plan_rather_than_guessing(self):
        ...
        idx = self.weekly_js.index("updatePlanTask(day.day_number")
        block = self.weekly_js[idx:idx + 900]
