...
HANDOFF.md item 1 is explicit that acceptance here has to be a measured
percentage over a labelled corpus, not "these three examples work now" -
three examples were exactly what was wrong with the matcher this
replaces (frontend/assets/js/coach-nlu.js). This test runs the REAL
frontend files under node (same pattern as
tests/test_badge_service.py::TestRegistryCoverage) so a change that
...
RUNNER = Path(__file__).resolve().parent / "js" / "coach_nlu_corpus_runner.js"
...
class TestSpecificRegressionsFoundDuringThisRewrite(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.script_prelude = (
            ...
            f"require({json.dumps(str(js_dir / 'coach-nlu.js'))});"
            f"require({json.dumps(str(js_dir / 'coach-knowledge.js'))});"
            f"require({json.dumps(str(js_dir / 'coach-knowledge-life.js'))});"
            f"require({json.dumps(str(js_dir / 'coach-chat.js'))});"
        )
