...
TEST_JWT_SECRET = "x" * 40  # >=32 chars, matches tests/test_tokens.py's own convention
...
class TestAuth(APITestCase):
    ...
    def test_forgot_password_for_registered_email_returns_a_usable_token(self):
        self._register(email="forgot@example.com", password="original123")
        response = self.client.post("/api/v1/auth/forgot-password", json={"email": "forgot@example.com"})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json()["reset_token"])

    def test_forgot_password_for_unknown_email_gives_the_same_message_but_no_token(self):
        known = self.client.post("/api/v1/auth/forgot-password", json={"email": "unknown-nobody@example.com"})
        self.assertEqual(known.status_code, 200)
        self.assertIsNone(known.json()["reset_token"])
    ...
    def test_reset_password_with_valid_token_logs_in_with_new_password(self):
        ...
        token = self.client.post("/api/v1/auth/forgot-password", json={"email": "reset@example.com"}).json()["reset_token"]
