...
Two rules, and they only work together.
  ...
  DAY LOCKING. Two gates, and a day must clear both: day N+1 opens once
  day N is fully ticked (the sequence), AND a day never opens before the
  calendar day it falls on. Enforced on the server, not only in the UI -
  a locked day that a direct PUT can tick anyway is not locked, and "you
  missed a day" only means anything if the days had to be done in order.

  The calendar half exists because the sequence alone let somebody tick
  all seven days in one evening and finish the week in ten minutes.
  These are a day's habits; a week nobody lived is not a week done.
...
Run: python3 -m unittest tests.wellness.test_plan_days_and_violations -v
"""
...
class TestUnlockedThrough(unittest.TestCase):
    ...
    def setUp(self):
        from services.wellness.violation_service import unlocked_through
    ...
    # ------------------------------------------------------ the calendar
    def test_the_calendar_holds_a_day_the_sequence_would_open(self):
        # Day 1 finished on the day the plan was made. The sequence says
        # day 2; the calendar says day 2 has not happened yet.
        today = date(2026, 3, 10)
        self.assertEqual(
            self.unlocked_through(
                {(1, 0), (1, 1)}, {1: 2, 2: 2},
                generated_on="2026-03-10", today=today,
            ),
            1,
        )

    def test_the_next_day_opens_the_next_day(self):
        self.assertEqual(
            self.unlocked_through(
                {(1, 0), (1, 1)}, {1: 2, 2: 2},
                generated_on="2026-03-10", today=date(2026, 3, 11),
            ),
            2,
        )

    def test_the_calendar_alone_never_opens_an_unfinished_day(self):
        # A week away and nothing ticked: the calendar would allow day 8,
        # but the sequence has not moved. The gates are an AND, and this
        # is the direction that would let somebody skip the whole week.
        self.assertEqual(
            self.unlocked_through(
                set(), {1: 2, 2: 2, 3: 2},
                generated_on="2026-03-10", today=date(2026, 3, 20),
            ),
            1,
        )

    def test_no_start_date_means_no_calendar_gate(self):
        # A plan stored before generated_on existed. Guessing a start
        # date would lock days on a date the app does not actually know,
        # so the calendar gate simply does not apply.
        self.assertEqual(
            self.unlocked_through({(1, 0), (1, 1)}, {1: 2, 2: 2}, generated_on=None),
            2,
        )

    def test_an_unparseable_start_date_does_not_lock_everything(self):
        self.assertEqual(
            self.unlocked_through({(1, 0), (1, 1)}, {1: 2, 2: 2}, generated_on="not-a-date"),
            2,
        )

    def test_a_plan_dated_in_the_future_still_opens_day_one(self):
        # Clock skew, or a hand-edited snapshot. Day 1 open is the floor;
        # zero open days is a state the UI has no sensible reading of.
        self.assertEqual(
            self.unlocked_through(
                set(), {1: 2, 2: 2},
                generated_on="2026-03-20", today=date(2026, 3, 10),
            ),
            1,
        )
...
class TestPlanDaysAndViolations(APITestCase):
    ...
    def test_finishing_day_one_does_not_open_day_two_the_same_day(self):
        # The whole point of the calendar gate. Somebody who ticks day
        # 1's three boxes in one sitting has done today's part, not two
        # days of it.
        plan = self._plan()
        self._finish_day(plan, 1)
        after = self._plan()
        self.assertEqual(after["unlocked_through"], 1)
        day_two = next(d for d in after["days"] if d["day_number"] == 2)
        self.assertTrue(day_two["locked"])
        self.assertEqual(self._tick(2, 0).status_code, 409)

    def test_a_day_held_only_by_the_calendar_says_so(self):
        # "Finish day 1 first" to somebody who just finished day 1 reads
        # as a bug. The card has to be able to tell the two gates apart.
        plan = self._plan()
        self._finish_day(plan, 1)
        after = self._plan()
        day_two = next(d for d in after["days"] if d["day_number"] == 2)
        self.assertEqual(day_two["lock_reason"], "calendar")
        day_three = next(d for d in after["days"] if d["day_number"] == 3)
        self.assertEqual(day_three["lock_reason"], "sequence")
        self.assertIsNone(after["days"][0]["lock_reason"])

    def test_finishing_day_one_opens_day_two_the_next_day(self):
        plan = self._plan()
        self._finish_day(plan, 1)
        # The plan started yesterday, so day 2 is today.
