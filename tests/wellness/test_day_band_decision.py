...
services/wellness/plan_lock_service.py. From the second logged day of a week
...
Run: python3 -m unittest tests.wellness.test_day_band_decision -v
"""
...
class TestDayBandDecision(APITestCase):
    ...
    def test_the_day_is_judged_against_the_other_days_not_itself(self):
        # With the day included, the centre would be (80 + 70) / 2 = 75
        # and 70 would sit comfortably inside its own band - the day
        # would be judging itself. Judged against the days it is being
        # compared TO, the centre is 80, which is the honest question:
        # "that is outside the range you have been running at".
        #
        # Asserted on the CENTRE rather than on band_low/band_high,
        # because the half-width is a prediction now
        # (services/ml/band_model_service) and this test is about which
        # days the centre is computed from - a property no model
        # touches. Pinning literal edges here would have made this test
        # fail every time the band model was retrained, while saying
        # nothing about what it is actually checking.
        self._seed([80.0, 70.0])
        status = self._status()
        centre = (status["band_low"] + status["band_high"]) / 2
        self.assertAlmostEqual(centre, 80.0, places=1)
    ...
    def test_an_exception_day_still_moves_the_band_but_less(self):
        # 80 and 45. Counted in full the centre is 62.5; at a quarter
        # weight it is (80*1 + 45*0.25) / 1.25 = 73.0. Neither 80 (which
        # would be erasure) nor 62.5 (which would be counting it).
        #
        # The centre is what the exception weight moves, and the centre
        # is what is asserted - the half-width around it comes from the
        # band model and is a separate question, pinned by
        # tests/ml/test_personal_band_model.py.
        self._seed([80.0, 45.0])
        self._decide("exception")
        plan = self._plan(regenerate=True)
        centre = (plan["band_low"] + plan["band_high"]) / 2
        self.assertAlmostEqual(centre, 73.0, places=1)
