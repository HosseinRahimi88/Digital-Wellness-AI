...
Run: python3 -m unittest tests.wellness.test_plan_recommendations_motivation -v
"""
...
from services.wellness.improvement_plan_service import (
    LANGUAGES,
    _THEME_I18N,
    _TIP_I18N,
    _TIER_LABEL_I18N,
...
RUNNER = paths.PROJECT_ROOT / "tests" / "js" / "motivation_runner.js"
...
class TestThePlanRendererReadsTheTranslations(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.js = (JS / "pages/weekly.js").read_text(encoding="utf-8")
...
class TestAdviceExistsForWhatTheModelActuallyFlags(unittest.TestCase):
    ...
    def test_the_at_risk_profile_now_gets_advice_for_every_harmful_factor(self):
        ...
        # NEVER_RECOMMEND is not a gap, it is a decision: non-actionable
        # inputs (age, weekday, a percentage against the user's own
        # baseline) and clinical instruments the app refuses to give
        # seven-day instructions about. Both lists are written down in
        # config/recommendation_registry.py with their reasons. Once the
        # score started counting screen load, SHAP began flagging
        # screen_vs_baseline_pct here for the first time - the answer is
        # to act on the minutes behind it, which ARE covered, not to
        # invent advice for a derived percentage.
        uncovered = [f.feature for f in harmful
                     if f.feature not in reg.FEATURE_RECOMMENDATIONS
                     and f.feature not in reg.NEVER_RECOMMEND]
        self.assertEqual(
            uncovered, [],
...
class TestTheDashboardShowsTranslatedRecommendations(unittest.TestCase):
    def test_the_dashboard_card_reads_text_i18n(self):
        js = (JS / "pages/dashboard.js").read_text(encoding="utf-8")
...
class TestMotivationalContent(unittest.TestCase):
    ...
    def test_no_fabricated_statistic_appears_in_any_fact(self):
        ...
        src = (JS / "features/motivation.js").read_text(encoding="utf-8")
    ...
    def test_the_coach_uses_it(self):
        menu = (JS / "coach/ai-menu.js").read_text(encoding="utf-8")
    ...
    def test_it_is_loaded_on_the_pages_that_use_it(self):
        for name in ("coach.html", "app.html", "dashboard.html"):
            with self.subTest(page=name):
                ...
                self.assertIn("assets/js/features/motivation.js", html)
