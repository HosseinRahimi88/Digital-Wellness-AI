...
These tests run the real frontend/assets/js/features/csv-library.js under node
...
RUNNER = paths.PROJECT_ROOT / "tests" / "js" / "csv_library_runner.js"
FRONTEND = REPO_ROOT / "frontend"
I18N = FRONTEND / "assets" / "js" / "core/i18n.js"
...
class TestCsvLibraryBehaviour(unittest.TestCase):
    ...
    def test_a_second_account_never_sees_the_first_ones_saved_days(self):
        """The reported bug, and the worst one in this file's area.

        The library used to key off one fixed localStorage string, so
        signing into a second account on the same machine listed the
        FIRST account's saved check-ins - somebody else's answers about
        their own sleep, stress and screen use, sitting under the new
        user's name and one click from refilling their form with it.
        """
        self.assertEqual(self.out["aliceSees"], ["Alice private day"])
        self.assertEqual(
            self.out["bobSeesAfterSwitch"], [],
            "signing in as a second account showed the first account's files",
        )

    def test_each_account_keeps_its_own_shelf(self):
        """Scoping must not be achieved by wiping - the first account's
        files have to survive the second account using the browser."""
        self.assertEqual(self.out["bobSees"], ["Bob own day"])
        self.assertEqual(
            self.out["aliceSeesAfterBob"], ["Alice private day"],
            "switching back found the first account's shelf emptied",
        )

    def test_signed_out_shows_nobody_s_files(self):
        self.assertEqual(self.out["signedOutSees"], [])

    def test_the_accounts_really_have_separate_keys(self):
        """A suffix nobody reads is not scoping. Two accounts, two keys."""
        self.assertEqual(
            self.out["storedKeys"],
            ["dwai_csv_library:user-alice", "dwai_csv_library:user-bob"],
        )
...
class TestCsvUiIsWiredAndTranslated(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.app_js = (FRONTEND / "assets" / "js" / "pages/app.js").read_text(encoding="utf-8")
    ...
    def test_the_library_script_is_loaded(self):
        self.assertIn('src="assets/js/features/csv-library.js"', self.app_html)
    ...
    def test_both_new_surfaces_have_a_guide_topic(self):
        controls = (FRONTEND / "assets" / "js" / "guide/guide-content-controls.js").read_text(encoding="utf-8")
