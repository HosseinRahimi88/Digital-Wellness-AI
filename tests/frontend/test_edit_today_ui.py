...
Server behaviour lives in tests/wellness/test_one_check_in_per_day.py. This file
...
Run: python3 -m unittest tests.frontend.test_edit_today_ui -v
"""
...
class TestEditTodayUI(unittest.TestCase):
    ...
    def test_the_tick_reaches_the_csv_upload(self):
        """The tick still governs a saving upload - it is just no longer
        the only thing the upload can be.

        The call used to be `importHistoryCsv(file, state.editToday)`.
        An upload is now a choice first (score it, or save it), so the
        flag passed is the one the ANSWER produced: choosing to replace
        days IS the edit consent, and the tick on the page is set from
        it rather than being a second control to find. What must not
        regress is that a saving upload still carries the flag and a
        preview never does.
        """
        self.assertIn("form.append('allow_update'", self.api_js)
        self.assertIn("window.DWApi.importHistoryCsv(", self.app_js)

        idx = self.app_js.index("window.DWApi.importHistoryCsv(")
        call = self.app_js[idx:idx + 260]
        self.assertIn("state.editToday", call, "the tick no longer reaches the upload")
        self.assertIn("dryRun", call, "a preview upload must be able to say it is one")

        # Choosing to replace sets the tick, rather than asking the user
        # to find it themselves - that WAS the reported bug.
        self.assertIn("state.editToday = true;", self.app_js)
        self.assertIn("editCheck.checked = true", self.app_js)
    ...
    def test_unticking_never_wipes_the_form(self):
        # The tick's own handler, not syncCheckInMode - the two live
        # next to each other now and only one of them may touch the
        # answers. Anchored on the addEventListener so this keeps
        # pointing at the handler if more is added around it.
        idx = self.app_js.index("editCheck.addEventListener('change'")
        handler = self.app_js[idx:idx + 800]
        code = re.sub(r"//.*", "", handler)
        self.assertIn("applyTodayAnswers()", code)
        self.assertIn("e.target.checked", code)
    ...
    def test_a_locked_tick_is_rendered_checked_and_unremovable(self):
        ...
        # `disabled` is now also set while "just testing" is on, so the
        # assertion is that it follows the lock - not that it is the
        # only thing that can set it.
        self.assertRegex(body, r"check\.disabled\s*=\s*[^;]*locked")
        self.assertIn("edit_today_note_locked", body)


    # --------------------------------- the two ticks are opposites
    def test_the_two_ticks_can_never_both_be_on(self):
        """The bug: a "just testing" run went out as an edit of today.

        The edit tick is force-ticked whenever today is already
        recorded, and it was independent of the "don't record this"
        tick. So the ordinary path - check in, come back, tick "just
        testing", submit - sent edit_today=true, and the user's real
        recorded day was replaced by a hypothetical they were only
        trying out.
        """
        self.assertIn("function syncCheckInMode", self.app_js)
        body = self.app_js[self.app_js.index("function syncCheckInMode"):]
        body = body[:body.index("function wireWizard")]
        code = re.sub(r"//.*", "", re.sub(r"/\*.*?\*/", "", body, flags=re.S))

        # Ticking "just testing" clears the edit tick, in state AND in
        # the DOM - a state flag the checkbox disagrees with is how the
        # user ends up looking at a tick that lies.
        self.assertIn("state.editToday = false", code)
        self.assertIn("editCheck.checked = false", code)
        # And the reverse.
        self.assertIn("state.excludeFromAnalysis = false", code)
        self.assertIn("excludeCheck.checked = false", code)

    def test_both_change_handlers_go_through_it(self):
        idx = self.app_js.index("function wireWizard")
        body = self.app_js[idx:idx + 2000]
        self.assertIn("syncCheckInMode('exclude')", body)
        self.assertIn("syncCheckInMode('edit')", body)

    def test_the_auto_lock_stands_down_while_testing(self):
        # refreshTodayCheckIn runs on every return to the form. If it
        # re-armed the edit tick regardless, the exclusion above would
        # be undone a moment after the user set it.
        body = self.app_js[self.app_js.index("async function refreshTodayCheckIn"):]
        body = body[:body.index("\n  /* Refills the form")]
        self.assertIn("state.excludeFromAnalysis", body)
        self.assertIn("!testing && editLockedToday()", body)

    def test_switching_to_testing_clears_an_untouched_prefill(self):
        # The other half: the edit tick fills the form with today's
        # answers, so a test that kept them returned the same score and
        # looked as though it had been counted as the real day.
        body = self.app_js[self.app_js.index("function syncCheckInMode"):]
        body = body[:body.index("function wireWizard")]
        self.assertIn("state.prefilledFromToday", body)
        self.assertIn("DWSchema.DEFAULTS", body)

    def test_an_edited_field_is_never_discarded(self):
        # Only an UNTOUCHED prefill may be cleared.
        body = self.app_js[self.app_js.index("function onFieldChange"):]
        body = body[:body.index("function updateDerivedPanel")]
        self.assertIn("state.prefilledFromToday = false", body)
    ...
    def test_a_wholly_blocked_upload_is_not_reported_as_a_broken_file(self):
        # `csv_import_none_imported` is now used twice: once in the
        # preview panel for a file that scored nothing, and once on the
        # save path. This test is about the SAVE path, so it anchors on
        # the last occurrence rather than the first - the earlier one is
        # a preview, which cannot be blocked by the duplicate-day rule
        # at all.
        idx = self.app_js.rindex("csv_import_none_imported")
