...
def _script_names(page: str) -> list[str]:
    ...
    # The folder is part of the path now (assets/js/core/shell.js), and
    # it is captured with the name: two modules in different folders are
    # different files, and a checker that flattened them would report a
    # page as loading something it does not.
...
class TestEveryLoggedInPageInitialisesTheShell(unittest.TestCase):
    def test_shell_js_is_loaded_on_every_logged_in_page(self):
        for page in LOGGED_IN_PAGES:
            with self.subTest(page=page):
                self.assertIn("core/shell.js", _script_names(page))
    ...
    def test_some_script_on_each_page_actually_calls_shell_init(self):
        ...
        # rglob, and the folder-qualified name: the modules live in
        # assets/js/<folder>/ now, so a glob of the top level finds
        # nothing and every page would look as if it never calls init.
        callers = {
            f"{p.parent.name}/{p.name}" for p in JS_DIR.rglob("*.js")
    ...
    def test_hall_page_specifically_initialises_the_shell(self):
        ...
        src = (JS_DIR / "pages/hall-page.js").read_text(encoding="utf-8")
    ...
    def test_no_page_controller_auto_greets_with_the_guide_on_load(self):
        ...
        for path in JS_DIR.rglob("*.js"):
            if path.name in {"guide/guide-tips.js", "guide/guide-click.js"}:
