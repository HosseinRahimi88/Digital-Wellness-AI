"""
Tests: the seven-day-ahead score (services/future_score_service.py).
...
Run: python3 -m unittest tests.test_future_score_service -v
"""
...
from services.future_score_service import FutureScoreService
...
class TestAugmentationEvidence(unittest.TestCase):
    ...
    def test_the_target_is_not_just_todays_score(self):
        """If it were, a model fitted to it would predict today and call
        it next week."""
        for name, stats in self.augmentation["splits"].items():
            self.assertLess(stats["corr_with_today"], 0.95, name)
...
class TestShippedEstimatorBeatsDoingNothing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        path = Path(__file__).resolve().parents[1] / "artifacts" / "metrics_future_regression.json"
        if not path.exists():
