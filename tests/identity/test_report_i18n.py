...
Run: python3 -m unittest tests.identity.test_report_i18n -v
"""
...
class TestRtlWrapping(unittest.TestCase):
    ...
    def setUp(self):
        if not report_language_support("fa")["supported"]:
            self.skipTest("shaping libraries unavailable in this environment")
        from services.identity.report_i18n import register_fonts
        register_fonts()

    def test_lines_stay_in_reading_order(self):
        from services.identity.report_i18n import FONT_REGULAR
