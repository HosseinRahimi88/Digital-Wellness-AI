...
Run: python3 -m unittest tests.test_coach_precision_and_menu -v
"""
...
RUNNER = Path(__file__).resolve().parent / "js" / "coach_precision_runner.js"
...
class TestOffTopicIsDeclined(unittest.TestCase):
    ...
    def test_the_known_collision_is_recorded_not_hidden(self):
        """"boiling" is two edits from "failing" (0.714) and the fuzzy
        floor is 0.7, which cannot be raised without losing
        "routine"/"routeen" at 0.71. Left in place deliberately: the
        alternative was gating the emotional intents on self-reference,
        and "feeling hopeless" carries none."""
        miss = self.report["knownMiss"]
        self.assertAlmostEqual(miss["similarity"], 0.714, places=3)
        # Recorded as the current truth. If a later change fixes it,
        # this flips and the comment above should be revisited.
        self.assertTrue(miss["stillAnswered"])
...
class TestTheCJKGuardIsTheRightShape(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.nlu = (JS / "coach-nlu.js").read_text(encoding="utf-8")
...
class TestMenuAnswers(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.menu = (JS / "ai-menu.js").read_text(encoding="utf-8")
...
class TestLapsedDemoTicksFormAPrefix(unittest.TestCase):
    """A locked day with every box ticked is not a state the app can
    reach on its own, and it reads as broken."""

    @classmethod
    def setUpClass(cls):
        cls.demo = (REPO_ROOT / "api" / "routers" / "demo.py").read_text(encoding="utf-8")

    def test_a_missed_day_stops_the_week_rather_than_leaving_a_hole(self):
        block = self.demo[self.demo.index("ticks: list[tuple[int, int]] = []"):]
        block = block[:block.index("progress_service.set_many_completed")]
        self.assertIn("stopped = True", block)
        self.assertIn("if stopped:", block)
...
class TestUpcomingDaysAreMarked(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.js = (JS / "dashboard.js").read_text(encoding="utf-8")
