...
Run: python3 -m unittest tests.test_startup_guard -v
"""
...
class TestTheClientDetectsIt(unittest.TestCase):
    ...
    def setUp(self):
        self.api = (JS_DIR / "api.js").read_text(encoding="utf-8")
        self.guard = (JS_DIR / "api-guard.js").read_text(encoding="utf-8")
...
class TestEveryPageLoadsTheGuard(unittest.TestCase):
    ...
    def _pages(self):
        return [
            ...
            if "assets/js/api.js" in p.read_text(encoding="utf-8")
        ]
    ...
    def test_each_of_them_loads_the_guard(self):
        for page in self._pages():
            self.assertIn("api-guard.js", page.read_text(encoding="utf-8"), page.name)

    def test_the_guard_loads_after_the_api_module_it_uses(self):
        for page in self._pages():
            source = page.read_text(encoding="utf-8")
            self.assertLess(
                source.index("assets/js/api.js"),
                source.index("assets/js/api-guard.js"),
                page.name,
            )
