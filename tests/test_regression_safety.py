...
class TestRegressionSafety(unittest.TestCase):
    ...
    def test_feature_schema_matches_trained_feature_columns(self):
        ...
        still exist in FEATURE_SCHEMA (schema was not narrowed/changed)."""
    ...
    def test_prediction_service_output_matches_direct_model_registry_output(self):
        ...
        *object lifecycle*, not *math*.
        ...
        import pandas as pd
        X_clf = pd.DataFrame([cleaned]).reindex(columns=clf_registry.feature_columns)
        X_reg = pd.DataFrame([cleaned]).reindex(columns=reg_registry.feature_columns)
