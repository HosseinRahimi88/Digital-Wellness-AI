...
(function () {
  ...
  function draw(canvas, points, opts) {
    ...
    /* Geometry kept on the element so the hover layer can hit-test
       ...
       change) automatically updates what hovering reports. Same contract
       as charts.js's _dwBars. */
    canvas._dwTrendPoints = all.map((p, i) => ({
      x: xAt(i),
      y: yAt(p.health_score),
      date: p.date,
  ...
  /* ---- hover read-out ---------------------------------------------
     The strip had no pointer handling at all: every other chart on the
     page explained itself on hover and this one, the wellness score over
     time, answered nothing. One listener per canvas, guarded by a flag,
     because draw() runs again on every resize and language change and
     binding each time would stack duplicate handlers.

     Follows charts.js's attachBarHover: the same .chart-tip element, the
     same wrap-relative positioning, so both charts behave identically. */
  function attachTrendHover(canvas) {
    if (canvas._dwTrendHoverBound) return;
    canvas._dwTrendHoverBound = true;
    ...
    canvas.addEventListener('pointermove', (e) => {
      const pts = canvas._dwTrendPoints;
      if (!pts || !pts.length) return hide();
      const r = canvas.getBoundingClientRect();
      const x = e.clientX - r.left;

      // Nearest point by x only. Hitting a 2px marker vertically is not
      // realistic, and the reader is asking "what happened on this day",
      // which is an x-axis question.
      let hit = pts[0];
      pts.forEach((pt) => {
        if (Math.abs(pt.x - x) < Math.abs(hit.x - x)) hit = pt;
      });
      if (Math.abs(hit.x - x) > 24) return hide();

      const t = (key, fallback) => (window.DWI18n ? window.DWI18n.t(key) : '') || fallback;
      const label = document.createElement('span');
      label.className = 'tip-label';
      label.textContent = hit.date || '';
      const value = document.createElement('span');
      value.className = 'tip-value';
      value.textContent = Math.round(hit.score * 10) / 10;

      tip.innerHTML = '';
      tip.appendChild(label);
      tip.appendChild(value);
      if (hit.isException) {
        // The one thing this chart knows that the number does not say:
        // this day was marked unusual and is not in the trend line.
        const note = document.createElement('span');
        note.className = 'tip-label';
        note.textContent = t('trend_tip_exception', 'exception day — not in the trend');
