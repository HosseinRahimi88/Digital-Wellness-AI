...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  /* ---- week menu ----------------------------------------------------
     A plan snapshot is stored per ISO week, so a 23-day demo leaves four
     of them - but until now only the week being lived was reachable and
     the earlier three were invisible. Chips are labelled "Week 0..n"
     rather than by ISO key, because "2026-W30" tells a reader nothing.

     Selecting a week reassigns `plan` and calls renderPlanBody(), which
     is the same path the tick handler already uses to redraw after the
     server changes something - so there is one renderer, not two. */
  async function renderWeekMenu() {
    const card = document.getElementById('planWeeksCard');
    const row = document.getElementById('planWeeksRow');
    if (!card || !row) return;

    let data;
    try {
      data = await window.DWApi.planWeeks();
    } catch (e) {
      return;
    }
    const weeks = (data && data.weeks) || [];
    // A menu of one is noise: that one week is the page already.
    if (weeks.length < 2) { card.classList.add('hidden'); return; }
    ...
    weeks.forEach((week) => {
      ...
      chip.type = 'button';
      chip.className = 'week-chip' + (week.is_current ? ' is-current' : '');
      chip.dataset.weekKey = week.week_key;

      const label = document.createElement('span');
      label.className = 'week-chip-label';
      label.textContent = `${window.DWI18n.t('plan_week_label') || 'Week'} ${week.index}`;
      chip.appendChild(label);

      const count = document.createElement('span');
      count.className = 'week-chip-count';
      count.textContent = `${week.completed_tasks}/${week.total_tasks}`;
      chip.appendChild(count);

      chip.addEventListener('click', async () => {
        Array.from(row.children).forEach((c) => c.classList.remove('is-selected'));
        chip.classList.add('is-selected');
        try {
          // The current week goes back to the live plan object rather
          // than the stored snapshot: only the live one carries the
          // lock state and violation counts the tick handler needs.
          plan = week.is_current ? livePlan : await window.DWApi.planWeek(week.week_key);
          renderPlanBody();
        } catch (err) {
    ...
    const current = row.querySelector('.week-chip.is-current') || row.lastElementChild;
    if (current) current.classList.add('is-selected');
    card.classList.remove('hidden');
  }

  await renderWeekMenu();
  ...
  function renderPlanBody() {
    ...
    plan.days.forEach((day) => {
      ...
      `;
      if (day.locked) {
        // Says WHY, and which of the two gates is holding the day.
        // "Locked" on its own reads as a fault in the app rather than
        // as the next step - and telling somebody to finish a day they
        // have already finished reads as a bug, which is what a single
        // message did once the calendar gate arrived.
        const note = document.createElement('p');
        note.className = 'day-locked-note';
        note.textContent = day.lock_reason === 'calendar'
          ? window.DWI18n.t('plan_day_calendar_note')
          : window.DWI18n.t('plan_day_locked_note').replace('{day}', plan.unlocked_through);
      ...
      day.tasks.forEach((task) => {
        ...
        row.querySelector('input').addEventListener('change', async (e) => {
          ...
          try {
            ...
            /* Every box on this day, now ticked. Read off the server's
               answer rather than off the checkboxes, because the tick
               that just happened is only real once the server has it -
               and celebrating a save that failed would be the app
               cheering for something that did not happen.

               The day number is the key, so re-reading the plan (which
               this handler does on every tick) cannot fire it twice for
               the same day, and un-ticking then re-ticking the last box
               does not either. */
            if (e.target.checked && window.DWCelebrate) {
              const fresh = (refreshed.days || [])
                .find((d) => d.day_number === day.day_number);
              const tasks = (fresh && fresh.tasks) || [];
              if (tasks.length && tasks.every((t) => t.completed)) {
                window.DWCelebrate.dayComplete(
                  `${refreshed.week_key}:${day.day_number}`,
                );
              }
            }
            ...
            if (err.code === 'plan_day_locked') {
              window.DWToast.error(day.lock_reason === 'calendar'
                ? window.DWI18n.t('plan_day_calendar_toast')
                : window.DWI18n.t('plan_day_locked_toast')
