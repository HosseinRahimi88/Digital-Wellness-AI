...
(function () {
  ...
  async function start(choice) {
    ...
    try {
      if (session.final_result) {
        localStorage.setItem('dwai_last_result', JSON.stringify(session.final_result));
      }
    ...
    window.DWApi.setToken(session.access_token);
  ...
  async function leave() {
    ...
    try {
      ...
      localStorage.removeItem('dwai_last_result');
  ...
  window.DWDemo = { run: openPicker, leave, isActive, renderBanner, state };
})();
