...
(function () {
  ...
  async function render(canvas) {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(fa ? 'سلامت دیجیتال هفته‌ی من' : 'My Digital Wellness Week', 70, 130);
    ctx.font = '400 26px Inter, sans-serif';
    ctx.fillStyle = 'rgba(234,252,246,.65)';
    ctx.fillText(new Date().toLocaleDateString(fa ? 'fa-IR' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }), 70, 172);
    ...
    ctx.fillText(fa ? 'امتیاز سلامت دیجیتال' : 'Wellness score', cx, cy + 78);

    // Stats row
    const stats = [
      { label: fa ? 'میانگین هفته' : 'Week avg', value: data.weekAvg != null ? Math.round(data.weekAvg) : '—' },
      { label: fa ? 'رکورد پیاپی' : 'Streak', value: data.streak != null ? `${data.streak}d` : '—' },
      { label: fa ? 'روزهای ثبت‌شده' : 'Days logged', value: data.entryCount != null ? data.entryCount : '—' },
    ];
    const statY = 740;
    const statW = (W - 140) / 3;
    ctx.textAlign = 'center';
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText(fa ? 'پرسونا' : 'Persona', 110, 900);
    ctx.font = '500 40px Inter, sans-serif';
    ctx.fillStyle = '#22f5c6';
    ctx.fillText(data.persona || (fa ? 'هنوز مشخص نشده' : 'Not yet determined'), 110, 950);

    ctx.font = '600 30px Sora, sans-serif';
    ctx.fillStyle = '#fff';
    ctx.fillText(fa ? 'مهم‌ترین عامل' : 'Top factor', 110, 1030);
    ctx.font = '500 36px Inter, sans-serif';
    ctx.fillStyle = 'rgba(234,252,246,.85)';
    const topLabel = data.topFeature ? String(data.topFeature).replace(/_/g, ' ') : (fa ? 'داده‌ی کافی نیست' : 'Not enough data');
    ...
    ctx.fillText(fa ? 'ساخته‌شده با Digital Wellness AI — امتیازها واقعی‌اند' : 'Made with Digital Wellness AI — real ML scores, not a template', W / 2, 1300);
