...
(function () {
  ...
  /* A small standalone four-language table (not part of COPY above) for
     ...
     tests/frontend/test_i18n_coverage.py's LANG_CONTAINER_BASELINE, which already
  ...
  /* The whole question, not either half. "How do I get better?" is the
     one people actually ask, and answering it with only the weak list
     leaves out the half that is cheapest to lose. Matched BEFORE the
     two half-questions above, because a phrase like "what should I do
     to improve" contains "what should i improve". */
  const SUGGESTIONS_PATTERNS = [
    /\b(improvement suggestions|suggestions to improve|how do i improve|how can i improve|how do i get better|what can i do better|give me suggestions|advice for me|what would you suggest|coach me)\b/i,
    /پیشنهاد.{0,14}بهبود|پیشنهاد.{0,10}(بده|بدی)|چطور بهتر شوم|چطور بهتر بشم|چیکار کنم بهتر|راهکار.{0,10}بده|توصیه.{0,10}(بده|بدی)/i,
    /اقتراحات التحسين|كيف أتحسن|كيف اتحسن|ماذا أفعل لأتحسن|انصحني/i,
    /改进建议|我该怎么变好|怎样才能更好|给我一些建议|给点建议/,
  ];
  ...
  /* Both halves at once, several ideas each - see
     assets/js/coach-suggestions.js for where the ideas live and why
     they are written rather than generated. */
  function suggestionsAnswer(full) {
    const P = (t) => (window.DWI18n && window.DWI18n.pick ? window.DWI18n.pick(t) : t.en);
    const tracks = full && full.plan_tracks;
    const built = window.DWCoachSuggestions
      ? window.DWCoachSuggestions.build(tracks)
      : null;
    if (built) return built;
    return P({
      en: "I could not read your plan just now, so I am not going to hand you advice with nothing behind it. Try again in a moment.",
      fa: 'الان نتوانستم برنامه‌ات را بخوانم، پس توصیه‌ای که پشتش چیزی نباشد به تو نمی‌دهم. کمی بعد دوباره بپرس.',
      ar: 'لم أستطع قراءة خطتك الآن، ولن أعطيك نصيحة بلا سند. جرّب بعد قليل.',
      zh: '我这会儿读不到你的计划，所以不会给你没有依据的建议。稍后再问一次。',
    });
  }
  ...
  function coachIntents() {
    ...
    coachIntentsCache = [
      ...
      { key: 'suggestions', keywords: kw(SUGGESTIONS_PATTERNS) },
  ...
  function respond(text, ctx, full) {
    ...
    if (matchesAny(SUGGESTIONS_PATTERNS, q, normQ)) {
      return { text: suggestionsAnswer(full), kind: 'answer' };
    }
    ...
    if (nlu) {
      ...
      if (coachHit.match) {
        if (coachHit.match.key === 'suggestions') return { text: suggestionsAnswer(full), kind: 'answer' };
        if (coachHit.match.key === 'plan_strengthen') return { text: strengthenAnswer(full), kind: 'answer' };
        if (coachHit.match.key === 'plan_maintain') return { text: maintainAnswer(full), kind: 'answer' };
    ...
    // Honest fallback (rule 5 in this file's header):
