...
(function () {
  ...
  const state = {
    ...
    // without it.
    todayCheckIn: null, editToday: false,
  };
  ...
  async function refreshTodayCheckIn() {
    ...
    const locked = editLockedToday();
    check.checked = locked;
    check.disabled = locked;
    state.editToday = locked;
    if (note) {
      note.textContent = window.DWI18n.t(locked ? 'edit_today_note_locked' : 'edit_today_note');
    }
  ...
  function wireForgotPassword() {
    ...
    const codeBlock = $('#resetCodeBlock');
    const codeDisplay = $('#resetCodeDisplay');
    const resetBtn = $('#resetSubmitBtn');
    ...
    sendBtn.addEventListener('click', () => withSubmitLock(sendBtn, async () => {
      ...
      if (!email) return;
      try {
        const res = await window.DWApi.forgotPassword(email);
        if (res.reset_token) {
          currentResetToken = res.reset_token;
          codeDisplay.textContent = window.DWI18n.t('auth_reset_code_shown').replace('{code}', res.reset_token);
          codeBlock.classList.remove('hidden');
          window.DWToast.success(res.message);
        } else {
          // Same generic message whether or not the email matched - the
          // response text alone never reveals which case happened.
          window.DWToast.info(res.message);
        }
      } catch (err) {
    ...
    resetBtn.addEventListener('click', () => withSubmitLock(resetBtn, async () => {
      const newPassword = $('#resetNewPasswordInput').value;
      if (!currentResetToken || !newPassword) return;
      try {
        const res = await window.DWApi.resetPassword(currentResetToken, newPassword);
  ...
  function maybeRunFirstRunTour() {
    ...
    // a screen still animating in.
    setTimeout(() => {
      try { window.DWGuide.startTour('onboarding', { force: true }); } catch (e) {}
    }, 700);
  }
  ...
  function wireCsvImport() {
    ...
    uploadInput.addEventListener('change', async (e) => {
      ...
      try {
        const result = await window.DWApi.importHistoryCsv(file, state.editToday);
        const parts = [];
