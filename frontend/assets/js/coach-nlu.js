/*
  ...
  "close enough". See HANDOFF.md item 1 for the full brief.
...
(function () {
  ...
  function classify(text, intents, opts) {
    ...
    const ranked = (intents || []).map((intent) => {
      const regexHit = intent.pattern && intent.pattern.test(raw) ? 1 : 0;
      let kwScore = 0;
      for (const kw of intent.keywords || []) {
        kwScore = Math.max(kwScore, phraseScore(qTokens, kw));
      }
