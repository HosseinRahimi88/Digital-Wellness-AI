...
(function () {
  ...
  /* ---- paper --------------------------------------------------------
     A page turning is not a tone, it is filtered noise with a shape:
     a burst that sweeps from low to high as the sheet passes, then
     dies. Oscillators cannot make that, so these three cues use the
     only noise source in this file - a short white-noise buffer pushed
     through a moving bandpass.

     Written for the book on the About page (assets/js/about-journal.js).
     Everything the rest of this engine guarantees still applies: they
     are silent when sound effects are off, they never fire before a
     real user gesture, and they are quieter under reduced motion. */
  function noiseBurst(opts) {
    ...
    band.frequency.setValueAtTime(opts.from || 700, t0);
    band.frequency.exponentialRampToValueAtTime(Math.max(60, opts.to || 2600), t0 + duration);

    const gain = c.createGain();
    const peak = Math.max(0.0002, opts.gain || 0.09);
    gain.gain.setValueAtTime(0.0001, t0);
    // A sheet is loudest as it passes, not as it starts.
    gain.gain.exponentialRampToValueAtTime(peak, t0 + duration * 0.35);
    gain.gain.exponentialRampToValueAtTime(0.0001, t0 + duration);

    source.connect(band).connect(gain).connect(master || c.destination);
    source.start(t0);
  ...
  /* One sheet turning. */
  function pageTurn() {
    if (throttled('pageTurn', 220)) return;
    noiseBurst({ duration: 0.34, from: 620, to: 3000, gain: 0.075, q: 0.8 });
    noiseBurst({ start: 0.1, duration: 0.2, from: 1800, to: 900, gain: 0.035, q: 1.4 });
  }

  /* A cover opening: heavier, slower, with the board settling after it. */
  function bookOpen() {
    if (throttled('bookOpen')) return;
    noiseBurst({ duration: 0.55, from: 260, to: 1500, gain: 0.09, q: 0.7 });
    tone(96, { type: 'sine', duration: 0.4, gain: 0.05, filterFreq: 400 });
    tone(148, { type: 'sine', start: 0.28, duration: 0.35, gain: 0.03, filterFreq: 600 });
  }

  /* And closing: the same board, landing. */
  function bookClose() {
    if (throttled('bookClose')) return;
    noiseBurst({ duration: 0.3, from: 1400, to: 220, gain: 0.08, q: 0.7 });
    tone(74, { type: 'sine', start: 0.16, duration: 0.32, gain: 0.055, filterFreq: 320 });
  }

  /* A page being written in - short, dry, and deliberately not a chime:
     the save already gets one from the toast. */
  function penStroke() {
    if (throttled('penStroke', 200)) return;
    noiseBurst({ duration: 0.16, from: 2400, to: 5200, gain: 0.03, q: 2.2 });
  }

  /* A kiss. Fires once, when the last exercise of a day is ticked.

     Built the same way the paper cues are, because a kiss is closer to
     paper than to a chime: it is a suction and a release, not a note.
     Three parts, in the order a real one happens - lips parting (a
     bright, very short noise crack), the suction (a bandpass sweeping
     DOWNWARDS, which is what makes it read as inward rather than as a
     hiss), and a small voiced "mwah" underneath so it lands as warm
     rather than as static.

     Deliberately not throttled against itself with a long window: it
     fires at most once per day completed, and a second one arriving is
     a second real event, not a repeat. */
  function smooch() {
    if (throttled('smooch', 600)) return;
    noiseBurst({ duration: 0.05, from: 2600, to: 5200, gain: 0.05, q: 1.6 });
    noiseBurst({ start: 0.03, duration: 0.2, from: 3200, to: 620, gain: 0.075, q: 2.6 });
    tone(392, { type: 'sine', start: 0.05, duration: 0.16, gain: 0.045, filterFreq: 1400 });
    tone(523.25, { type: 'sine', start: 0.13, duration: 0.2, gain: 0.035, filterFreq: 1800 });
  }
  ...
  window.DWSound = {
    ...
    pageTurn, bookOpen, bookClose, penStroke, smooch,
