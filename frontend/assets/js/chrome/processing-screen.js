...
(function () {
  ...
  const STEPS = {
    ...
    /* The first line names the demo the user actually picked. There are
       four lengths (3/7/15/23), so a fixed "23 days" was wrong three
       times out of four. `{days}` is substituted from opts.days. */
    demo: {
      en: [
        'Building {days} days of realistic check-ins…',
        'Running each day through the real model…',
        'Assigning a persona from the pattern…',
        'Connecting demo friends in the League…',
        'Wiring up your Coach, plans, and analytics…',
      ],
      fa: [
        'در حال ساخت {days} روز بررسی واقع‌گرایانه…',
        'اجرای هر روز روی مدل واقعی…',
        'تعیین پرسونا از روی الگو…',
        'وصل‌کردن دوستان نمایشی در لیگ…',
        'آماده‌سازی مربی، برنامه‌ها و تحلیل‌ها…',
      ],
      ar: [
        'بناء {days} يوماً من الفحوصات الواقعية…',
        'تشغيل كل يوم عبر النموذج الحقيقي…',
        'تعيين شخصية من النمط…',
        'ربط أصدقاء تجريبيين في الدوري…',
        'تجهيز مدربك وخططك وتحليلاتك…',
      ],
      zh: [
        '正在构建 {days} 天的真实检测…',
        '正在对每一天运行真实模型…',
  ...
  /* Persian and Arabic read their own digits; Chinese and English use
     Western ones. The count is a number the user just chose, so it is
     shown in the script they are reading the sentence in. */
  const DIGITS = {
    fa: ['۰', '۱', '۲', '۳', '۴', '۵', '۶', '۷', '۸', '۹'],
    ar: ['٠', '١', '٢', '٣', '٤', '٥', '٦', '٧', '٨', '٩'],
  };

  function localiseNumber(value, lang) {
    const digits = DIGITS[lang];
    const plain = String(value);
    if (!digits) return plain;
    return plain.replace(/[0-9]/g, (d) => digits[Number(d)]);
  }

  function stepsFor(flow, vars) {
    const lang = (window.DWI18n && window.DWI18n.get()) || 'en';
    const set = STEPS[flow] || STEPS.predict;
    const lines = set[lang] || set.en;
    if (!vars || vars.days === undefined || vars.days === null) return lines;
    const days = localiseNumber(vars.days, lang);
    return lines.map((line) => line.replace('{days}', days));
  }
  ...
  // The screen has always been dismissible by click, Esc, Enter or
  // Space, and it still is - what went is the small grey line that said
  // so. It sat under every prediction telling the reader how to leave a
  // screen they had not yet decided they wanted to leave, which is a
  // strange first thing to say to somebody waiting on their own result.

  /**
   * @param {Promise} work  the real request; the screen waits on it.
   * @param {object}  opts  { flow: 'predict'|'coach'|'demo', days }
   *                        `days` fills the {days} slot in the demo
   *                        script, so the screen names the demo that is
   *                        actually running.
  ...
  function run(work, opts) {
    ...
    const steps = stepsFor(flow, { days: opts.days });
