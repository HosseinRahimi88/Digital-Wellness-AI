...
(function () {
  ...
  /* The sound-effects toggle used to be appended to the music widget as
     a bare speaker icon, right beside the player's own volume speaker.
     Two near-identical glyphs, one of which changes the volume and one
     of which silently flips a preference that only makes a sound when
     something else triggers one - reported as "there is a button that
     does nothing", and fairly.

     It was never the music player's setting. It lives in the settings
     panel, labelled, where it always also lived; this only stops it
     being duplicated as an unlabelled icon. Removing it is also what
     frees that slot in the widget for the track controls the player
     actually needed. */
  function removeStraySoundFxWidgetButton() {
    const stray = document.getElementById('soundFxToggleBtn');
  ...
  function init() {
    ensureNavButtons();
    removeStraySoundFxWidgetButton();
  ...
  window.DWChrome = {
    ...
    removeStraySoundFxWidgetButton, ensureNavButtons, wireNotifBell, refreshNotifBadge,
  };
})();
