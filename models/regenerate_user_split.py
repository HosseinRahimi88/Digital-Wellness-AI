...
Fixes the leakage documented in docs/reports/ML_AUDIT_REPORT.md section 1b/3: the
...
Run once: `python3 -m models.regenerate_user_split`
...
REPORT_FILE = paths.PROJECT_ROOT / "artifacts" / "leakage_verification_report.json"
...
def _load_pool() -> pd.DataFrame:
    """The three splits pooled, with exact duplicate rows dropped.

    WHY THE DEDUP. Measured on the files this was last run against, the
    pool of 104,509 rows held only 90,368 distinct ones - 16,501 of the
    duplicates inside train.csv alone. A diary row that appears twice is
    not two days of evidence; it silently doubles one respondent's
    weight in every fit, and it makes every row count in every report
    an overstatement. Dropping them costs 13.5% of the pool and buys a
    number that means what it says.

    Deduplication also has to happen BEFORE the group key is built, not
    after: the key is derived from the row, so two identical rows always
    land in the same group, and a group carrying its own duplicates
    would drag them into whichever split it went to.
    """
    ...
    before = len(pooled)
    # Canonicalised through a CSV round-trip BEFORE deduplicating.
    #
    # pandas' default float formatting is not fully round-trippable: two
    # rows that differ in the last bits of a float are distinct in
    # memory, survive drop_duplicates(), and then land on identical text
    # once written. Measured on this pool, an in-memory dedup reported
    # complete success and the files still came back with 17 duplicate
    # rows in them.
    #
    # Round-tripping first makes the values in memory exactly the values
    # the CSV will hold, so the dedup is exact and the later write
    # cannot reintroduce anything.
    with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as handle:
        canonical_path = handle.name
    try:
        pooled.to_csv(canonical_path, index=False)
        pooled = pd.read_csv(canonical_path)
    finally:
        os.unlink(canonical_path)
    pooled = pooled.drop_duplicates().reset_index(drop=True)
    logger.info(
        "Pooled %d rows, %d distinct (%d exact duplicates dropped)",
        before, len(pooled), before - len(pooled),
    )
    return pooled
