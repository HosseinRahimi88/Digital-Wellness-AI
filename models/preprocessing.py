...
def get_feature_columns(df: pd.DataFrame, task: str = "classification") -> List[str]:
    ...
    leakage_columns = [
        ...
        # The seventh component, added by models/data_loader.py. It is a
        # deterministic function of the minute fields, so the model can
        # and should learn its effect from those - but the column itself
        # is a piece of the target and would be pure leakage as a
        # feature.
        ...
        "night_usage_burden",
        # `day_index` is the position of a row inside a synthetic user's
        # generated diary, and the generator wrote each user's trajectory
        # against day number - so it predicts future_health_class_7d well
        # while meaning nothing for a real person, whose "day 15" says
        # nothing about their wellness next week. It ranked #2 by
        # permutation importance and was inflating validation accuracy by
        # about two points (0.8044 -> 0.7827 once removed). A plain
        # cumulative counter reproduced the same effect far harder on its
        # own (0.8044 -> 0.9099), which is what exposed the mechanism.
        # Measurements: models/research_classification_trend.py.
        # It is still needed to ORDER each user's rows when building trend
        # features, which is why the CSVs keep it - it is just never a
        # feature.
        "day_index",
    ]
    ...
    #
    # Empirically validated by retraining. The current numbers are in
    # artifacts/metrics.json and artifacts/metrics_regression.json; the
    # "before" comparison was measured against the pre-selection
    # artifacts during development and is quoted below rather than
    # kept, since a second set of pickles in the tree would only ever
    # go stale. Classification improved on every
