...
| `app/pages/Login.py` | صفحه لاگین/ثبت‌نام |

### فاز ۲ — Onboarding
| فایل | چیکار می‌کنه |
|---|---|
| `app/pages/Onboarding.py` | ۶ سوال (هدف، دلیل استفاده، برنامه هفتگی، خواب، سطح تلاش، رضایت‌نامه) |
...
| `app/pages/Prediction.py` | تب "Quick Demo" اضافه شد کنار فرم اصلی |

### فاز ۴ — Dashboard (صفحه جدید)
| فایل | چیکار می‌کنه |
|---|---|
| `app/pages/Dashboard.py` | امتیاز آخرین چک-این، مقایسه با قبلی، heatmap هفتگی، نقاط قوت/ضعف از SHAP، توصیه‌ها |

### فاز ۵ — Weekly Plan (پایدارسازی)
| فایل | چیکار می‌کنه |
|---|---|
| `services/plan_progress_service.py` | تیک‌زدن تسک‌های پلن هفتگی، **ذخیره واقعی** (نه فقط session) |
| `app/pages/Weekly_Insights.py` | بخش "Weekly Plan" پایدار اضافه شد |

### فاز ۶ — Recommendations (محتوای Guardrail)
| فایل | چیکار می‌کنه |
|---|---|
| `config/recommendation_registry.py` | هر توصیه یه `success_metric` و `safety_note` گرفت |
| `app/components/recommendation_card.py` | نمایش این محتوا در UI |

### فاز ۷ — Analytics (روند تاریخی)
| فایل | چیکار می‌کنه |
|---|---|
| `app/pages/Analytics.py` | نمودار روند امتیاز در طول زمان + الگوی روز هفته، از داده واقعی |

### فاز ۸ — Profile Management (صفحه جدید)
| فایل | چیکار می‌کنه |
|---|---|
| `app/pages/Profile.py` | مدیریت اکانت، ویرایش ترجیحات، لینک به Model Performance واقعی |
...
قبل از هر کار توسعه‌ای بیشتر، پروژه رو با `streamlit run app/Home.py` توی محیط واقعی خودت (با اینترنت، بعد از `pip install -r requirements.txt`) اجرا کن و از نظر UX/ظاهر چک کن. من نمی‌تونم این بخش رو ببینم چون Streamlit واقعی توی sandbox من نصب نمیشه.
