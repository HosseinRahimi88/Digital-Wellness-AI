...
| `app/pages/Login.py` | Login/register UI |
| `config/demo_profiles.py` | Canonical "Quick Demo" profiles, built from A's real `FEATURE_SCHEMA` |
| `app/pages/Dashboard.py` | New unified dashboard (A had none) |
...
| `app/pages/Onboarding.py` | 6-question preference onboarding flow |
| `app/pages/Profile.py` | Account + preference management |
...
| `app/Home.py` | Nav links to new pages, login-state greeting |
| `app/pages/Prediction.py` | Added "Quick Demo" tab; refactored the render logic into a shared function (no behavior change) |
| `app/pages/Weekly_Insights.py` | Added persistent Weekly Plan section |
| `app/components/improvement_plan_card.py` | Added `render_persistent()` alongside the original (untouched) `render()` |
...
| `app/components/recommendation_card.py` | New "Success metric & safety" expander |
| `app/pages/Analytics.py` | Added real historical trend charts (Score History, Day-of-Week Pattern) from `HistoryService` |
...
`services/model_service.py`, `services/storage/`, `app/pages/AI_Coach.py`,
`app/pages/Model_Performance.py`, `app/pages/About.py`, `app/pages/What_If_Simulator.py`,
